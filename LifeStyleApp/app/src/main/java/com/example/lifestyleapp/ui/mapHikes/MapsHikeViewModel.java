...
import com.example.lifestyleapp.repositories.MapsHikeRepository;
...
public class MapsHikeViewModel extends AndroidViewModel {
        MutableLiveData<Boolean> threadCompleted = new MutableLiveData<>();
    MapsHikeRepository mapsRepo;
    ArrayList<Trail> trails;

    public MapsHikeViewModel(@NonNull Application application) throws IOException, JSONException {
        super(application);

        mapsRepo = new MapsHikeRepository(application);
    }

    void setUserLocation(Location location) throws IOException, JSONException {
        mapsRepo.setUserLocation(location);
        mapsRepo.getThreadCompleted().observeForever(threadObserver);
    }

    Observer<Boolean> threadObserver = new Observer<Boolean>() {
        @Override
        public void onChanged(Boolean aBoolean) {
            threadCompleted.setValue(aBoolean);
            if (aBoolean==true)
                trails = mapsRepo.getTrails();
        }
    };

    MutableLiveData<Boolean> getThreadCompleted(){
